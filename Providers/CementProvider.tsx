...
export const CementContext = createContext<CementProducsState | undefined>(
  undefined,
);

export const useCementContext = () => {
  const context = useContext(CementContext);
  if (!context) {
    throw new Error('useCementContext must be used within an CementProvider');
  }
  return context;
};

interface CementProviderProps {
  children: React.ReactNode;
}

export const CementProvider: React.FC<CementProviderProps> = ({children}) => {
  ...
  return (
    <CementContext.Provider value={value}>{children}</CementContext.Provider>
  );
};
